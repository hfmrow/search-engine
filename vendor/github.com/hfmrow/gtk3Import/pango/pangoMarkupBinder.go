...
func (pm *PangoMarkup) MarkupSeparate() (outStrSl []string) {
	...
	for _, mType := range pm.markTypes {
		outStrSl = append(outStrSl, markupSeparate(text, mType...)...)
	}
	copy(pm.OutStringSl, outStrSl)
	pm.finalizeSeparate()
	return pm.OutStringSl
}
...
func (pm *PangoMarkup) prepare() {
	pm.InString = strings.Replace(pm.InString,
		pangoEscapeChar[1][0], pangoEscapeChar[1][2], -1)
	pm.InString = strings.Replace(pm.InString,
		pangoEscapeChar[0][0], pangoEscapeChar[0][2], -1)
}

// Escape special characters after marking ("<", "&")
func (pm *PangoMarkup) finalize() {
	pm.OutString = strings.Replace(pm.OutString,
		pangoEscapeChar[1][2], pangoEscapeChar[1][1], -1)
	pm.OutString = strings.Replace(pm.OutString,
		pangoEscapeChar[0][2], pangoEscapeChar[0][1], -1)
}

// Escape special characters after marking ("<", "&"), SEPARATE version
func (pm *PangoMarkup) finalizeSeparate() {
	for idx, _ := range pm.OutStringSl {
		pm.OutStringSl[idx] = strings.Replace(pm.OutStringSl[idx],
			pangoEscapeChar[1][2], pangoEscapeChar[1][1], -1)
		pm.OutStringSl[idx] = strings.Replace(pm.OutStringSl[idx],
			pangoEscapeChar[0][2], pangoEscapeChar[0][1], -1)
	}
...
func markup(text string, mType ...string) string {
	...
	case 2:
		...
		return fmt.Sprint(markupType[mType[0]][0], mType[1],
			markupType[mType[0]][1], text, markupType[mType[0]][2])
	case 3:
		// i.e: markup("display", "stc", "double", "red")
		return fmt.Sprint(markupType[mType[0]][0], mType[1],
			markupType[mType[0]][1], mType[2], markupType[mType[0]][2],
			text, markupType[mType[0]][3])
	default:
...
// Apply pango markup format to text. They can be combined.
// Output will give []sting containing each markup sections separatly.
func markupSeparate(text string, mType ...string) []string {
	switch len(mType) {
	case 1:
		// i.e: markup("display", "sub")
		return []string{markupType[mType[0]][0], text, markupType[mType[0]][1]}
	case 2:
		// i.e: markup("display", "stc", "red")
		return []string{markupType[mType[0]][0], mType[1],
			markupType[mType[0]][1], text, markupType[mType[0]][2]}
	case 3:
		// i.e: markup("display", "stc", "double", "red")
		return []string{markupType[mType[0]][0], mType[1],
			markupType[mType[0]][1], mType[2],
			markupType[mType[0]][2], text, markupType[mType[0]][3]}
	default:
